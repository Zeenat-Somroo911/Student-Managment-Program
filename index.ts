...
while (true) {
  ...
  if (answer.option === "Exit") {
    console.log(
      ...
      chalk.greenBright.bold.italic(
        "Thank you for using the Student Management Program.DEVELOPED BY : ZEENAT SOMROO. GOODBYE HAVE A NICE DAY!"
      )
    );
    break;
  }
  ...
  if (answer.option === "Add Student") {
    ...
    let newStudent: Student = new Student(answer1.name); // passing [answer1.name = any name written in message ] to the counstructor to create a object with thw help pf class
    console.log(
      chalk.yellow("---------------------------------------------------------")
    );
    console.log(
      "Dear",
      chalk.magenta.bold.italic(newStudent.name),
      "you are added sucssesfully & Your ID number is = " +
    ...
    for (let i = 0; i < kind.length; i++) {
      // looping through the kind array which is present at the top of the code . to get access of every element inside of object .
       ...
       if (answer2.number == kind[i].id) {
        ...
        console.log(
          "Dear",
          chalk.blue.bold.italic(kind[i].name),
          `you have successfully enrolled in "${chalk.cyanBright.bold(
            answer3.course
          )}" course `
        );
        console.log(
      ...
      if (answer4.number === kind[i].id) {
        ...
        console.log(
          `Dear ${chalk.blueBright.bold(
            kind[i].name
          )} your current balance is : ${chalk.green(kind[i].balance)}$`
        );
        console.log(
          chalk.yellow(
            "-------------------------------------------------------------"
    ...
    for (let i = 0; i < kind.length; i++) {
      ...
      if (answer5.number === kind[i].id) {
        ...
        if (answer6.amount > kind[i].balance) {
          ...
          console.log(
            ` ${chalk.blueBright.bold(kind[i].name)} has paid : ${chalk.green(
              answer6.amount
            )}$ tution fee`
          );
          console.log(
  ...
  if (answer.option === "View status") {
    ...
    for (let i = 0; i < kind.length; i++) {
      ...
      if (answer7.number === kind[i].id) {
        ...
        console.log(
          ` Name = ${chalk.cyanBright(kind[i].name)} \n ID = ${chalk.cyanBright(
            kind[i].id
          )} \n Balance = ${chalk.cyanBright(
            kind[i].balance
